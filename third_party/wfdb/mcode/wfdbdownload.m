...
%Fast path: if the record exists locally (absolute or relative to current
%working directory), skip cache download probes.
local_record_exists = false;
local_candidates = {recordName, strrep(recordName,'/','\'), strrep(recordName,'\','/')};
for ci = 1:length(local_candidates)
    candidate = local_candidates{ci};
    if(isempty(candidate))
        continue;
    end
    if(isfile([candidate '.hea']) || isfile([candidate '.dat']))
...
if(local_record_exists || ~isempty(file_info) || isempty(ind) || (config.CACHE==0))
    success=-1;
else
