...
def calculate_r_amplitude_neurokit(ecg_signal: np.ndarray, lead_idx: int, sampling_rate: int = 500) -> Dict[str, Optional[float]]:
    try:
...
def calculate_s_amplitude_neurokit(ecg_signal: np.ndarray, lead_idx: int, sampling_rate: int = 500) -> Dict[str, Optional[float]]:
    try:
        lead_signal = ecg_signal[:, lead_idx]
        ecg_cleaned = nk.ecg_clean(lead_signal, sampling_rate=sampling_rate)
        _, rpeaks_dict = nk.ecg_peaks(ecg_cleaned, sampling_rate=sampling_rate)
...
def calculate_q_amplitude_neurokit(ecg_signal: np.ndarray, lead_idx: int, sampling_rate: int = 500) -> Dict[str, Optional[float]]:
    try:
        lead_signal = ecg_signal[:, lead_idx]
        ecg_cleaned = nk.ecg_clean(lead_signal, sampling_rate=sampling_rate)
        _, rpeaks_dict = nk.ecg_peaks(ecg_cleaned, sampling_rate=sampling_rate)
...
def load_mimic_record_index(
    ...
    try:
        ...
        if cohort_path is not None and cohort_path.exists():
            query = f"""
            WITH cohort AS (
                SELECT DISTINCT subject_id, study_id, hadm_id, primary_label
                FROM read_parquet('{str(cohort_path).replace("'", "''")}')
            )
            SELECT b.*, c.hadm_id, c.primary_label
            FROM ({base_select}) b
            JOIN cohort c
        ...
        return df.drop_duplicates(subset=["subject_id", "study_id"]).reset_index(drop=True)
    finally:
        con.close()
...
def extract_neurokit2_feature_row(ecg_signal: np.ndarray, sampling_rate: int) -> Dict[str, float]:
    ...
    for lead in r_leads:
        result = calculate_r_amplitude_neurokit(ecg_signal, LEAD_MAP[lead], sampling_rate=sampling_rate)
        row[f"R_Amp_{lead}"] = result["value"]
        row[f"R_Amp_{lead}_count"] = result["count"]
        row[f"R_Amp_{lead}_iqr"] = result["iqr"]

    for lead in s_leads:
        result = calculate_s_amplitude_neurokit(ecg_signal, LEAD_MAP[lead], sampling_rate=sampling_rate)
        row[f"S_Amp_{lead}"] = result["value"]
        row[f"S_Amp_{lead}_count"] = result["count"]
        row[f"S_Amp_{lead}_iqr"] = result["iqr"]

    for lead in q_leads:
        result = calculate_q_amplitude_neurokit(ecg_signal, LEAD_MAP[lead], sampling_rate=sampling_rate)
        row[f"Q_Amp_{lead}"] = result["value"]
        row[f"Q_Amp_{lead}_count"] = result["count"]
        row[f"Q_Amp_{lead}_iqr"] = result["iqr"]

    for lead in st_leads:
        result = calculate_st_elevation_neurokit(ecg_signal, LEAD_MAP[lead], sampling_rate=sampling_rate, st_offset_ms=60)
...
def _save_checkpoint(rows: List[Dict[str, object]], checkpoint_dir: Path, processed_count: int) -> None:
    checkpoint_dir.mkdir(parents=True, exist_ok=True)
    checkpoint_path = checkpoint_dir / f"hybrid_192_checkpoint_{processed_count}.parquet"
    pd.DataFrame(rows).to_parquet(checkpoint_path, index=False)
    print(f"\n✓ Checkpoint saved: {checkpoint_path} (rows={processed_count:,})")
...
def run_matlab_ecgdeli(
    ...
    if matlab_batch_command:
        ...
        command_text = (
            ...
            f"{script_name}('{index_csv_escaped}','{output_csv_escaped}',{workers});"
        )
    ...
    log_file = open(matlab_log_path, "w", encoding="utf-8", errors="replace")
    ...
    while process.poll() is None:
        ...
        if latest_processed_from_log is not None:
            if latest_total_from_log and pbar is not None and pbar.total in (None, 0):
    ...
    class _CompletedProcessResult:
        def __init__(self, returncode: int, log_path: Path) -> None:
            self.returncode = returncode
            self.log_path = log_path

    completed = _CompletedProcessResult(process.returncode or 0, matlab_log_path)

    if completed.returncode != 0:
        details = ""
        try:
            tail_lines = completed.log_path.read_text(encoding="utf-8", errors="replace").splitlines()[-30:]
            details = "\n".join(tail_lines).strip()
        except Exception:
            details = ""
        raise RuntimeError(
            "MATLAB ECGDeli extraction failed "
            f"(exit code {completed.returncode}). See log: {completed.log_path}. "
            f"Last lines:\n{details}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--matlab-checkpoint-every", type=int, default=50, help="MATLAB ECGDeli progress/checkpoint interval in records (lower = more live updates)")
...
def main() -> None:
    ...
    if args.run_matlab_ecgdeli:
        ...
        if args.ecgdeli_csv is None:
            args.ecgdeli_csv = args.matlab_output_csv
            print(f"Using MATLAB ECGDeli output as source CSV: {args.ecgdeli_csv}")

    ecgdeli_df = load_ecgdeli_features(
        source_csv=args.ecgdeli_csv,
    ...
    if max(1, args.parallel_processes) > 1:
        ...
        with ProcessPoolExecutor(max_workers=max(1, args.parallel_processes)) as executor:
            ...
            for idx, result in enumerate(progress_iter, start=1):
                ...
                if args.checkpoint_every > 0 and idx % args.checkpoint_every == 0:
                    _save_checkpoint(rows, checkpoint_dir, idx)
                if not interactive_tty and idx % max(1, args.tqdm_miniters) == 0:
                    print(f"Extracting NeuroKit2 features (parallel): {idx}/{len(records)}")
    else:
        progress_iter = tqdm(
            records,
            total=len(records),
        ...
        for idx, record in enumerate(progress_iter, start=1):
            ...
            if args.checkpoint_every > 0 and idx % args.checkpoint_every == 0:
                _save_checkpoint(rows, checkpoint_dir, idx)
            if not interactive_tty and idx % max(1, args.tqdm_miniters) == 0:
                print(f"Extracting NeuroKit2 features: {idx}/{len(records)}")
